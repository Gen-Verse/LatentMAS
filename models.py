...
class ModelWrapper:
    def __init__(self, model_name: str, device: torch.device, use_vllm: bool = False, args = None):
        ...
        if self.model_backend == "vlm" and use_vllm:
            raise ValueError("Qwen3.5-0.8B VLM text-first support uses the Hugging Face backend; --use_vllm is not supported.")
        ...
        if self.model_backend == "vlm":
            if AutoModelForImageTextToText is None:
                raise ImportError(
                    "AutoModelForImageTextToText is required for Qwen3.5-0.8B VLM support. "
                    "Upgrade transformers to a version that provides image-text-to-text auto models."
                )
            self.processor = AutoProcessor.from_pretrained(model_name)
            self.tokenizer = getattr(self.processor, "tokenizer", None)
            if self.tokenizer is None:
                raise RuntimeError("The VLM processor does not expose a tokenizer for text-only LatentMAS prompts.")
            _ensure_pad_token(self.tokenizer)
            with torch.no_grad():
