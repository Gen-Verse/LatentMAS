"""
Surgical MRRE Pipeline Runner: Hidden-State Mapping, Fit Surgical MRRE, IFL Evaluation, Report.

Usage:
    python scripts/run_surgical_pipeline.py --config configs/surgical_smoketest.yaml
    python scripts/run_surgical_pipeline.py --config configs/surgical_smoketest.yaml --stages A,B
    python scripts/run_surgical_pipeline.py --config configs/surgical_smoketest.yaml --model aisingapore/Llama-SEA-LION-v3-8B-IT --device cuda:0
    python scripts/run_surgical_pipeline.py --config configs/surgical_smoketest.yaml --resume
    python scripts/run_surgical_pipeline.py --config configs/surgical_smoketest.yaml --dry-run
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the Surgical MRRE pipeline.",
    ...
    parser.add_argument("--output-dir", type=Path, default=None, metavar="DIR", help="Override output_dir.")
...
def _bootstrap_logging(log_dir: Optional[str], level: str = "INFO") -> None:
    ...
    if log_dir:
        ...
        fh = logging.FileHandler(Path(log_dir) / f"surgical_pipeline_{ts}.log")
...
def main() -> int:
    ...
    logger.info("=" * 70)
    logger.info("Surgical MRRE Pipeline Runner  v%s", __version__)
    ...
    if args.dry_run:
        ...
        try:
            from mrre_drift.pipeline.surgical_pipeline import SurgicalPipeline  # noqa: F401
    ...
    try:
        from mrre_drift.pipeline.surgical_pipeline import SurgicalPipeline
    except ImportError as exc:
        logger.error("Failed to import SurgicalPipeline: %s", exc)
    ...
    try:
        pipeline = SurgicalPipeline(config=cfg, resume=args.resume)
